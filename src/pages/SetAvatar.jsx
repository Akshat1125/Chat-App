...
import { setAvatarRoute } from '../../Backend/APIRoutes';
...
export default function SetAvatar() {
  ...
  //constants
    ...
    const toastOption={
      ...
      const fetchAvatar=useCallback(async(iscancelled)=>{
        if(!iscancelled){
        const data = [];
        for (let i = 0; i < 4; i++) {
      ...
      const setProfilePic=async()=>{
        if(selectedAvatar===undefined)toast.error("Please select an avatar",toastOption);
        else{
      ...
      (!avatars)?(<Container>
       ...
       <Container>
        <div className="title-container">
            <h1>Pick an avatar for ur profile picture</h1>
        </div>
